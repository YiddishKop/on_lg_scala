...
for(v <- vects) {
    do somthing with whole object v
}

for(Vect3D(x,y,z) <- vects){
    do somthing with all attrs of object  x, y, z
}

for(Vect3D(x,_,_) <- vects){
    do somthing with some attrs of object x  
}

for(v@Vect3D(x,_,_) <- vects){
    do somthing with whole object v, and its attr x.  
}